  ...
  //      echo "Connection successful\n";
    ...
    if ($result->num_rows > 0) {
     ...
     //   echo "0 results";
      ...
      //customer went on website
      $stmtcheck = $con->prepare("SELECT * FROM Contacts 
        WHERE C_UserID = ? 
        AND Branch_no = ?;");
        $stmtcheck->bind_param("ss", $_SESSION["UserID"], $_SESSION["Branch_no"]);
        $stmtcheck->execute();
        ...
        if($result->num_rows > 0) {
         ...
         //       echo "CustID: " . $row["C_UserID"] . "<br>";
          ...
          //      echo "not in contacts";
                ...
                VALUES (?, ?);");
                $stmt->bind_param("ss", $_SESSION["UserID"], $_SESSION["Branch_no"]);
                $stmt->execute();
...
<title>Canada Wide Car Rental Service - Customer</title>
</head>
...
<form action="veh_search_cust_post.php" method="post">
  <tr>
    <th width = 25%>
    <div class = "searchbar">
    ...
    <button class= "addbutton" text-align=right type="button" onclick="window.location.href='cust_view_res.php'"> View Reservations</button> 
    </th>
  </tr>
  </table>
  ...
  <!--<form action='cust_veh_post.php' method='post'>-->
